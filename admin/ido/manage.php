...
<?php 
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * from ido where id = '{$_GET['id']}' ");
...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		...
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="ido"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=ido">Cancel</a>
		</div>
	</div>
</div>
...
<script>
	...
	$(document).ready(function(){
		$('.select')
		$('#ido').submit(function(e){
			e.preventDefault();
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Content.php?f=ido",
				...
				success:function(resp){
					if(resp != undefined){
						...
						if(resp.status == 'success'){
							location.href=_base_url_+"admin/?page=ido";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
							end_loader();
