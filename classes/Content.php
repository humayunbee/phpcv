...
Class Content extends DBConnection {
	...
	public function ido(){
		...
		if(empty($id)){
			$sql ="INSERT INTO ido set $data";
		}else{
			$sql ="UPDATE ido set $data where id = {$id}";
		}
		$save = $this->conn->query($sql);
		$action = empty($id) ? "added":"updated";
		if($save){
			$resp['status']='success';
			$resp['message']= " IDO Details successfully ".$action;
			$this->settings->set_flashdata('success',$resp['message']);
	...
	public function ido_delete(){
		extract($_POST);
		$qry = $this->conn->query("DELETE FROM ido where id = $id");
		if($qry){
			$resp['status']='success';
